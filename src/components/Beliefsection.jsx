...
import GoogleReview from './GoogleReview'; // <-- import your GoogleReview component
  ...
  return (
    <div
      className={`flex items-center space-x-3 p-4 sm:p-6 md:p-8 rounded-3xl transition-all duration-500 ease-out transform hover:scale-105 hover:shadow-lg ${
        isBlack ? 'bg-black text-white' : 'bg-white text-gray-800 border border-gray-800'
      }`}
      style={{
        borderRadius: '24px',
    ...
    >
      ...
      >
        <Icon className="w-6 h-6 sm:w-8 sm:h-8 text-gray-800" />
      </div>
      <div className="flex-1">
        <h3 className="text-lg sm:text-xl font-medium mb-2 sm:mb-3">{title}</h3>
        <p className="text-sm sm:text-base md:text-lg leading-relaxed">{description}</p>
      </div>
    </div>
  );
};
    ...
    <div className="w-full bg-white py-12 sm:py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8" ref={sectionRef}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          {/* LEFT COLUMN */}
          <div className="order-1 lg:order-1 flex flex-col justify-start space-y-10 lg:space-y-14">
            
            {/* Heading */}
            <h1
              className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-medium leading-snug text-gray-900"
              style={{
                opacity: isVisible ? 1 : 0,
            ...
            <div className="max-w-lg">
              ...
              >
                Our beliefs aren't just words; they are the foundation of every adventure we offer. With high commitment
                to sustainability, authenticity, and customer-centricity, we ensure that every trip you take with us is
                valuable.
              </p>
            </div>

            {/* Google Review Box — animated same as beliefs */}
            <div
